...
GatewayLorawanMac::Send (Ptr<Packet> packet)
{
  ...
  macHdr.SetMType(LorawanMacHeader::MType::UNCONFIRMED_DATA_UP); // Set as unconfirmed data up (non-ACK) so to get "false" in https://github.com/signetlabdei/lorawan/blob/e8f7a21044418e92759d5c7c4bcab147cdaf05b3/model/class-a-end-device-lorawan-mac.cc#L157C1-L157C1 as in https://github.com/signetlabdei/lorawan/blob/e8f7a21044418e92759d5c7c4bcab147cdaf05b3/model/lorawan-mac-header.cc#L145
  
  packetCopy->AddHeader(macHdr);

  std::cout << "GatewayLorawanMac::Send::MType: " << (int)macHdr.GetMType() << std::endl;
  // ---------------------------------------------------------------------//
  ...
  // ---------------------------------------------------------------------//
  // m_phy->Send (packetCopy, params, frequency, sendingPower);
  // ---------------------------------------------------------------------//
...
// void
// GatewayLorawanMac::Send2 (Ptr<Packet> packet)
// {
//   NS_LOG_FUNCTION (this << packet);
